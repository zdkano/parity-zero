...
### ProviderReviewItem / ProviderReview (ADR-044, ADR-045, ADR-046)
Structured review output from provider invocations. Each `ProviderReviewItem` carries kind, category, title, summary, paths, confidence, evidence, and source. Items are validated, normalised, deduplicated, and bounded (max 8). A `ProviderReview` container is carried on `ReasoningResult` and `AnalysisResult`. Review items are **non-authoritative** — they do not create findings, affect scoring, or change the decision. **Provider review is the primary non-authoritative review surface (ADR-045).** When present, provider review supersedes both heuristic concerns/observations and legacy candidate notes in the markdown output. Concerns and observations become a fallback shown only when no provider review exists. **Evidence discipline (ADR-046):** After parsing, items undergo post-parse filtering — speculative missing-control claims without evidence are suppressed or softened to `review_attention`, test/fixture noise is suppressed, non-security commentary is removed, items without evidence are bounded to `low` confidence, and overlapping weak items are collapsed. Provider requests now include larger code excerpts (2500 chars) with natural boundary truncation and related code context for route/controller groupings. **Fuller bounded changed-file context (ADR-047):** Provider requests include full file content for small relevant files (under 3000 chars), file-level context annotations, and a deterministic change summary.

### Change Summary (ADR-047)
Short deterministic "What Changed" section generated from changed file paths, review bundle metadata, and plan signals. Factual and compact — describes what changed (routes, auth, models, etc.) without judgment. Rendered in markdown between the decision/risk score and the provider review section. Not included in `ScanResult` JSON.
