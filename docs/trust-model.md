...
- Provider requests include **bounded code evidence** from `ReviewBundle` items (ADR-043, ADR-046) — actual changed code excerpts (up to 2500 chars per target) for the most security-relevant files, with natural boundary truncation and related code context for route/controller groupings. This makes provider reasoning more evidence-based but does **not** change trust semantics. Provider output remains non-authoritative regardless of the evidence quality provided to it.
- **Evidence discipline (ADR-046)** — after parsing, provider items undergo post-parse evidence-discipline filtering. Speculative missing-control claims without code evidence are suppressed or softened. Test/fixture noise, filename-only guesses, and non-security commentary are suppressed. Items without evidence are bounded to `low` confidence. This makes provider output more trustworthy as a review surface but does **not** change its non-authoritative status.
